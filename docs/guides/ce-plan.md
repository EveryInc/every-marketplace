...
| `use fable` / `have opus plan this` | Elevate the interpret-findings-then-author step and explicitly requested Bake-off candidates to that model. Also settable as `plan_model: <model>` in CE config. A prompt request overrides the config key. |
