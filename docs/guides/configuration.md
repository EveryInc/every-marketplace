...
- **A pack file without frontmatter or without `applies_when`** — skipped; `ce-plan` warns once naming the file (the pack's top-level `README.md` is skipped silently).
- **A rule-shaped file inside a subdirectory of a pack** — never read; the resolver warns once per pack naming the folder. Where things go is the guide's [Pack layout](./packs.md#pack-layout).
