...
describe("ce-code-review contract", () => {
  ...
  test("Stage 4 collects by observed return shape and fails closed without a collector", async () => {
    ...
    expect(skill).toMatch(/every.*successful launch.*terminal result payload/i)
    ...
    expect(content).toMatch(/blocking collector/i)
    expect(content).toMatch(/until every.*successful.*launch.*terminal result payload/i)
    expect(content).toMatch(/terminal.*tool error.*malformed.*failed reviewer/i)
    expect(content).toMatch(/no live path that can match them to a terminal result payload/i)
    ...
    expect(solution).toMatch(/every successful launch can be matched to a terminal result payload/i)
