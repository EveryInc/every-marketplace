...
1. **Puffery.** "Pivotal moment", "testament to", "setting the stage", "deeply rooted", "evolving landscape". Cut the claim of importance and state what happened.
...
11. **Avoiding "is".** "Serves as", "stands as", "boasts", "features", "represents", "marks". Write "is" or "has".
...
15. **False ranges.** "From X to Y" where X and Y are not on a scale. List the items.
...
18. **Propped adverbs.** "Runs quickly" is "is fast" or the number. "Significantly improves" is the measured delta. An adverb holding up a weak verb means the verb is wrong.
19. **Nominalizations.** "The demotion of the grid is the change" is "demote the grid". Prefer the verb.
20. **Abstract metaphor nouns in technical prose.** Substrate, wedge, vector, nexus, primitive as a noun, harness as a metaphor, surface as in "API surface", scaffolding as a metaphor, ratchet, evacuate for moving code, endgame, north star, flywheel. Use the concrete word: base, add, method, a limit that only tightens, move out, the last phase.
21. **Feeling instead of mechanism.** "The database stays close at hand", "types that follow your schema". Name what the reader can do or check: "a column rename fails the build".

## Structure

22. **Dense sentences.** A sentence the reader parses twice. One idea per sentence; a chain of semicolons becomes a list.
23. **Passive hiding the actor.** "Queries are validated" is "the compiler validates queries". Keep passive only when the actor is unknown or beside the point.
...
30. **Bold label that restates its line.** "**Performance:** performance improved." Convert to prose. A bold lead-in that names the item and is followed by new detail stays.
...
36. **Chatbot openers and closers.** "Great question", "Certainly", "I hope this helps", "let me know if", "want me to". Remove; state the content.
