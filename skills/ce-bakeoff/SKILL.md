...
This is experimental. Direct use is available; `ce-plan` and `ce-brainstorm` integrate it only when explicitly requested. Do not turn a routine choice into an automatic competition.
...
For direct use, deliver the result in chat. If the user requested a retained document or the artifact is too substantial for chat, use the project's established durable artifact location and provide its path. Resolve configured artifact conventions before writing; do not invent a second canonical plan. Keep the complete result accessible to its consumer before clearing run scratch, using the active environment's permitted cleanup mechanism.
