...
- Set `autofix_class` from who can choose the fix and permission to edit, independently of severity:
  ...
  - `gated_auto`: A specific fix that changes meaning, supported by the document and project evidence, within existing permission. Several technical fixes may work; recommend the smallest one that solves the problem. Show it with the other proposed edits for approval before applying it.
