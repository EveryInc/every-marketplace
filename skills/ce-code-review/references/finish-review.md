...
Invoke the `ce-noslop` skill in author mode before writing human-readable findings, and write them under its tests; if the harness cannot reach that skill, write under those tests directly: say what a thing does not how it feels, cut anything that could move to another project unchanged, name the actor of every verb, one idea per sentence, lead with the decision, and gloss any identifier a reader without the document open could not act on. Preserve exact code identifiers, error text, and every distinct consequence or required action.
