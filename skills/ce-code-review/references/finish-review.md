...
A matched Compound Pack rule that the `learnings-researcher` reports the diff **contradicts** is a finding candidate, not research — it enters the same gates every structured reviewer's finding passes (quote-the-line, validation, the false-positive check), and a claim whose rule text does not actually reach the quoted line is dropped like any other false positive. For each candidate that holds, before the first helper run, serialize it as a complete compact reviewer return in the shape `references/dispatch-reviewers.md` defines (every field the helper requires, or it lands in `malformed_findings`), with reviewer `learnings`, `file`/`line`/`first_evidence` from the contradicting changed line it quoted, severity by the rule's consequence per `references/action-class-rubric.md`, and anchor 100 only when the researcher's output quotes both the rule text and the changed line (otherwise 75). Its `evidence` — the rule's quoted text and the `(pack: <id>, <path within the pack>)` citation — goes in the source-detail map entry so hydration carries it. Include it in the helper input with the structured returns so it receives a stable `#` and the ordinary actionable route; when a structured reviewer reports the same defect and fix path under different wording, step 1 below reconciles them into one finding that keeps the citation — a `project-standards` finding on the same line with different rule text and its own fix stays a separate `#`. A rule violated only by an unchanged line the researcher quoted is a pre-existing candidate: serialize it the same way with `pre_existing: true`, so it lands in the pre-existing partition rather than the actionable queue. Rules the diff honors and relevant past solutions stay advisory for Stage 6 section 7.
...
7. **Learnings & Past Solutions.** Surface the `learnings-researcher` results that are not findings: relevant past solutions, and matched pack rules the diff honors, as "Known Pattern" notes with links to `<root>/solutions/` files or the `(pack: <id>, <path within the pack>)` citation. A pack rule the diff contradicts entered the finding set in Stage 5 and appears in the findings tables, not here.
