...
A matched Compound Pack rule that the `learnings-researcher` reports the diff **contradicts** is a review finding, not research: before the first helper run, serialize each one as a compact reviewer return (reviewer `learnings`; `file`, `line`, and `first_evidence` from the contradicting changed line it quoted; `evidence` carrying the rule's quoted text and its `(pack: <id>, <path within the pack>)` citation; severity by the rule's consequence per `references/action-class-rubric.md`; anchor 100 only when both the rule and the line are quoted, otherwise 75) and include it in the helper input with the structured returns, so it receives a stable `#`, deduplicates against a structured reviewer that found the same defect, and takes the ordinary actionable route. Rules the diff honors and relevant past solutions stay advisory for Stage 6 section 7.
