...
Then apply only the judgment the helper cannot own. Treat every reviewer output as a claim: verify its consequence against the intended outcome and the current source. A concrete maintenance benefit can justify a finding without a runtime bug, but preference, speculative completeness, and reviewer agreement cannot. Resolve disputes from evidence before elevating an essential user decision.
...
4. **Soft-bucket demotion before validation.** After the final helper pass, inspect both surviving `findings` and the remaining `suppressed_findings`. Reject claims with no established material consequence or worthwhile benefit, even when mechanics promoted their confidence. Record their disposition internally; do not recycle rejected claims into residual risks or testing gaps. Retain an uncertain risk only when project evidence establishes its relevance. Keep every `settled_conflict`-stamped finding primary as required by Stage 5 step 2.
...
7. **Collect coverage and advisory assets.** Keep helper drop/suppression counts. Admit persona-supplied residual risks and testing gaps under the same consequence and relevance boundary as the primary findings; do not preserve rejected claims through these fields. Deduplicate surviving concerns and preserve any selected learnings, agent-native, and deployment-verification outputs. Schema drift from `data-migration` is already in the merged finding set.
