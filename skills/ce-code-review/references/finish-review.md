...
Invoke the `ce-noslop` skill in author mode before writing human-readable findings, and write them under its tests; if the harness cannot reach that skill, write plainly: one idea per sentence, name the actor, lead with the decision. Preserve exact code identifiers, error text, and every distinct consequence or required action.
