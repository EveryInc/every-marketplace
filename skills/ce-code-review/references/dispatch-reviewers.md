...
**Agent lifecycle.** Collect each local agent's terminal outcome before releasing it, including failed outcomes. Once collected, promptly close or release agents owned by this review when the active harness exposes that operation and assigns it to the caller; finish that cleanup before refilling slots, advancing stages, or returning. This applies to validators as well as reviewers. Do not send acknowledgement or status messages to completed agents that have no remaining work. Completion or interruption alone is not proof that capacity was released. When release is harness-managed or unavailable, do not invent a shutdown command or claim cleanup succeeded; report any retained-capacity limitation and follow the existing fallback or failure contract instead of retrying indefinitely.
