...
Collect by the primitive's observed return, not its host name or requested background setting. Collection completes when every successful launch can be matched to a terminal result payload. Consume valid compact JSON from that payload, including an in-band return, and classify a terminal tool error or malformed output as a failed reviewer under the degraded-coverage rules below. A launch receipt is uncollected, not a reviewer return. A progress or status notification is not a terminal result even when it names the reviewer. The usual asynchronous success is a host-native terminal answer that names the launched work and carries the result payload. An ID-addressed blocking collector is another sufficient path, not a required one. Do not infer either path from a tool name. When dispatch returns an asynchronous id or receipt, wait on the live path until every successful launch can be matched to a terminal result payload. These harness-managed waits gather the batch; they are not the forbidden detached-delegate poll loop. Stage 5 must never run on a partial roster. When the asynchronous primitive keeps a completed reviewer in its concurrency slot until it is closed, close every collected reviewer before refilling and before leaving Stage 4; no collected reviewer may starve the Stage 5b validator batch.

If launched reviewers have no live path that can match them to a terminal result payload, stop the launched work and take the mode's failure path instead of waiting on unmatched notifications, emitting progress, or synthesizing a partial roster. Hand every persisted peer to its owning cleanup in `references/cross-model-review.md` before taking that failure path; no return may abandon detached work. In `mode:agent`, emit only `{"status":"failed","reason":"<one sentence>"}`.
