...
Collect by the primitive's observed return, not its host name or requested background setting. Collection completes when every successful launch reaches a terminal outcome. Validate each outcome: consume valid compact JSON whether returned in-band or collected asynchronously, and classify a terminal tool error or malformed output as a failed reviewer under the degraded-coverage rules below. A launch receipt is uncollected, not a reviewer return: when dispatch returns an asynchronous id or receipt, use the host's native waiting and result delivery until every successful launch reaches a terminal outcome and its result is consumed or classified as failed. Waiting and result delivery may be separate capabilities. A notification alone is not a completed review; a delivered terminal result can complete collection. These harness-managed waits are not the forbidden detached-delegate poll loop. Stage 5 must never run on a partial roster.

If launched reviewers have no reliable collection path that establishes both terminal state and result, stop the launched work and take the mode's failure path. Do not substitute progress-only output or a partial roster for completion. Hand every persisted peer to its owning cleanup in `references/cross-model-review.md` before taking that failure path; no return may abandon detached work. In `mode:agent`, emit only `{"status":"failed","reason":"<one sentence>"}`. Judge the collection path by the live host contract, not a tool name or a requirement that one tool both wait and return the result.
