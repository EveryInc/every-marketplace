...
6. **Stage 4.** Read `references/dispatch-reviewers.md`. Dispatch the materialized local roster as one foreground concurrent batch sized to the host's active-agent cap. Every successful launch is collected when a terminal result payload can be matched to it. Consume valid compact returns, classify a terminal tool error or malformed output as a failed reviewer, and keep launch receipts uncollected. A progress or status notification is not a terminal result. The usual asynchronous success is a host-native terminal answer that names the launched work and carries the result payload. Do not synthesize until every successful launch is collected. If launched work cannot be matched to a terminal result payload, stop it, discharge any persisted peer through its owning cleanup before returning the failure result, and never emit progress or wait on unmatched notifications. Detaching local review into a polled background job is forbidden. The cross-model peer is the only detached work, and it may overlap this batch.
