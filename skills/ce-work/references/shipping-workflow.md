...
3. **Code Review**
   ...
   **3b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: adjudicate findings against evidence and scope, batch warranted fixes by file, dispatch fix subagents. Orchestrator merges, tests, commits. Then proceed to the Residual Work Gate.
...
4. **Residual Work Gate** (REQUIRED when `ce-code-review` ran and left warranted unresolved findings)

   Reconcile the review's Actionable Findings against followup dispositions and the authorized outcome. Rejected claims are closed, not residual work. Continue fixes needed to complete that outcome without asking for authority already granted.

   When an unresolved concern prevents the requested outcome or shows a settled decision cannot work, it is never auto-accepted as a residual. Resolve it within existing authority, or return `status: blocked` with the missing evidence or user decision, its consequence, and a recommended resolution. Autonomous runs return the blocker rather than invent consent. Group related user-owned decisions.

   A proposed action outside the required outcome stays unapplied when its evidence or authority is missing; that does not block independent authorized work. Nonblocking residuals do not require an interactive routing menu. Record the warranted concern, why it is deferred, and its review-run context before Final Validation. Use the authorized PR's `## Unapplied review findings` section, or `references/tracker-defer.md` on an authorized tracker-defer path. If neither sink is authorized or reachable, return the residuals verbatim and state they are recorded nowhere else. Recording is disclosure, not permission to execute the deferred action.

   Skip the gate when there are no warranted residuals or dedicated review was skipped. A reported count alone does not decide whether to stop.
