...
- **Skip and report malformed pack files.** A pack file with no YAML frontmatter or no `applies_when` is skipped; list every skipped file once under a `Skipped pack files` line in the output so the author can fix it. A top-level `README.md` is the pack's description rather than a rule, so it is skipped without being listed.
