...
Save the full reviewer responses and the decisions already made alongside the identity of the reviewed document, so a later interactive review can resume. `ce-doc-review` decides whether those results can still be used. Keep its non-interactive result for the summary above the post-generation menu:
