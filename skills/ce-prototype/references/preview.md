...
Pick the browser that will load the prototype — one surface. For an annotated preview, use the returned `authorize_url`, not the public `url`; the browser must establish its session cookie before it can annotate. A plain preview still uses `url`.

To select an initial screen or application state, set the authorization link's optional `next` query parameter to the complete origin-root-relative destination, including its query and fragment. Encode it as one parameter, for example:

```js
const handoff = new URL(authorizeUrl); // the helper's returned authorize_url
handoff.searchParams.set("next", "/nested/checkout.html?variant=b&token=demo#details");
```

Use `handoff.href` for that browser. Without `next`, the destination is `/`. Absolute URLs, protocol-relative URLs (`//host`), backslashes, and control characters are refused. Keep application query/hash state inside `next`; do not replace the authorization link's `token` or append the application's fragment directly to the bootstrap URL. The authorization page replaces its own URL with the selected same-origin destination, preserving the application's own `token` and other state without copying the bootstrap credential into it.

When that browser is on this machine, keep the returned host. When it is not, start with `--host 0.0.0.0` and rewrite only the handoff URL's host to one the explorer can reach; preserve its authorization path and complete query, including `token` and any encoded `next`. Do not also hand localhost. Wait talks the bind address with the file token.
