...
**Prose.** Invoke the `ce-noslop` skill in author mode before composing, and write framing and connective prose under its tests; if the harness cannot reach that skill, write plainly: one idea per sentence, name the actor, lead with the decision. Identifiers, paths, protocols, and error text stay where they *are* the claim or the review target — do not dilute mechanism language into vague plain English.
