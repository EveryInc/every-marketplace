...
**Prose.** Invoke the `ce-noslop` skill in author mode before composing, and write framing and connective prose under its tests; if the harness cannot reach that skill, write under those tests directly: say what a thing does not how it feels, cut anything that could move to another project unchanged, name the actor of every verb, one idea per sentence, lead with the decision, and gloss any identifier a reader without the document open could not act on. Identifiers, paths, protocols, and error text stay where they *are* the claim or the review target — do not dilute mechanism language into vague plain English.
